...
def getExpiredUsers(conn)
	return conn.query("select user_id, user_name from users where status is null");
end
...
users_arr.each { |user_rec|
	...
	begin
		t_user = twitter.follow(u_id)
		#puts "update users set status ='expired' where user_id =#{u_id}"
		conn.query("update users set status ='invited', invited_time = curdate() where user_id =#{u_id}")
	rescue Exception => e
